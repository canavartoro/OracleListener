...
ORDER BY WH.WHOUSE_CODE");
        ...
        private void CheckItemMField()
        {
            Task.Run(() =>
            {
                Logger.I($"{Environment.GetFolderPath(Environment.SpecialFolder.Templates)}\\CheckItemMField.log");
                if (!File.Exists($"{Environment.GetFolderPath(Environment.SpecialFolder.Templates)}\\CheckItemMField.log"))
                {
                    using (OracleProvider db = new OracleProvider())
                    {
                        ...
                        {
                            db.Execute(@"CREATE SEQUENCE  ""UYUMSOFT"".""ZFIND_SAGE_HESAPPLANI_ID""  MINVALUE 1 MAXVALUE 999999999999999999999999999 INCREMENT BY 1 START WITH 1 NOCACHE  NOORDER  NOCYCLE");
  ...
  PRIMARY KEY(""HESAPPLANI_ID"")
        ...
        private void txtIrsaliyeNo_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
